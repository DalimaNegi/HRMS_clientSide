...
function Dashboard() {
  ...
  useEffect(() => {
    axios
      ...
      .catch((err) => {
        let { success, message, data } = err.response.data;
        if (success === false) {
          alert(message);
        }
      });
  ...
  let totalEmployees = empFormData.length;
  //console.log(totalEmployees);

  //considering this for date of joining .
  ...
  // let recentEmployees = empFormData.filter((item) => {
  //   let emp_date = new Date(item.DOB);
  //   return emp_date >= five_days_ago;
  // });

  let adults = new Date();
  adults.setFullYear(adults.getFullYear()-20);

  let recentEmployees = empFormData.filter((item)=>{
    let empDOB = new Date(item.DOB);
    return empDOB < adults;
  });
